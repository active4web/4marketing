...
$result['date_title']=$page->date_title;
...
$this->load->library('form_validation');
$this->form_validation->set_rules('token_id', lang('Customer ID'), 'trim|required');
$this->form_validation->set_rules('advertsing_id',lang('advertsing_id'), 'trim|required|numeric|min_length[1]|max_length[1]');
$this->form_validation->set_rules('content', lang('Content'), 'trim|required');
if($this->form_validation->run() === FALSE){
if(form_error('token_id')) {
if($this->input->post('token_id')==="" || !$this->input->post('token_id')){
...
if($this->input->post('city_id')!=""){
  $this->db->like('city_id', $this->input->post('city_id'));
  }
...
$result['id']=(int)$page->id;
$result['category_id']=$page->cat_id;
$category_name=get_table_filed('category',array('id'=>$page->cat_id),"name");
if($category_name!=""){
...
$user_email=get_table_filed('customers',array('id'=>$bage->user_id),"email");
$user_phone=get_table_filed('customers',array('id'=>$bage->user_id),"phone");
...
public function add_advertising(){
      ...
      $this->checkLang($lang);
        ...
        $this->form_validation->set_rules('city', lang('City id'), 'trim|required');
...
$this->api_return([
        ...
        'status' => false
          ...
          $expire_date=date('Y-m-d', strtotime($Date. " + $time_days days"));
...
get_img_config_insert('images','uploads/products/',$file,$file_name,'image','gif|jpg|png|jpeg',600000,600000,600000,array('id'=>$advertising_id),"600","450",0,$advertising_id);
    ...
    $data['details'][]= $result;
    $this->api_return([
                  'message' => lang('تم تعديل الاعلان بنجاح'),
                  'errNum' => 405,
                  'status' => true,
                  "result" => $data
                ],200);
...
$customers_id=get_table_filed('customers',array('id'=>$customer_id),"id");
...
public function add_soical(){
      ...
      $this->checkLang($lang);
        ...
        $this->form_validation->set_rules('sex', lang('sex'), 'trim|required');
        $this->form_validation->set_rules('age_f', lang('age_f_error'), 'trim|required');
		    $this->form_validation->set_rules('places', lang('places_error'), 'trim|required');
...
if(form_error('sex')){
if($this->input->post('sex')==="" || !$this->input->post('sex')){
$data[] = array('message'=> strip_tags(lang('sex_error')),"errNum" =>3);
}
}
if(form_error('age_f')){
if($this->input->post('age_f')==="" || !$this->input->post('age_f')){
$data[] = array('message'=> strip_tags(lang('age_f_error')),"errNum" => 5);
}
}
            
          
if(form_error('places')){
if($this->input->post('places')==="" || !$this->input->post('places')){
$data[] = array('message'=> strip_tags(lang('places_error')),"errNum" =>6);
}
}
