...
$this->load->view("index/include/footer",$data);
  ...
  function city(){
    ...
    if($featured==""||$featured=="yes"){
    $total=$this->db->get_where($tables,array('special'=>'1','city_id'=>$ID,'delete_key'=>'1','expired_date'=>'1','view'=>'1'))->result(); 
    $arr_condition= array('special'=>'1','city_id'=>$ID,'delete_key'=>'1','expired_date'=>'1','view'=>'1');
      if(count($total)==0){$arr_condition= array('city_id'=>$ID,'delete_key'=>'1','expired_date'=>'1','view'=>'1');}
    }
    else{
      $arr_condition= array('city_id'=>$ID,'delete_key'=>'1','expired_date'=>'1','view'=>'1');
    }


    $config = array();
    $config['base_url'] = base_url().'cat/city'; 
    $config['total_rows'] = $this->data->record_count($tables,$arr_condition,'',$arr,$arr_type);
    $config['per_page'] =33;
    $config['full_tag_open'] = '<ul class="pagination">';
    $config['full_tag_close'] = '</ul>';   
...
$this->load->view("index/include/head",$data );
$this->load->view("index/include/header",$data );
$this->load->view('city',$data_conent);
$this->load->view("index/include/footer",$data);
  }
