      ...
      <div class="row">
        <div class="col-xs-12">
          <ul>
            ...
            <?php 
$id_cat=get_table_filed('department',array('id'=>base64_decode($this->input->get("ID"))),"id_cat");
...
<div class="home-tab">
   ...
   <div class="page-title">
              <h2>
<?= 
get_table_filed('department',array('id'=>base64_decode($this->input->get("ID"))),"name");
?>
 </h2></div>

<?php
if(count($dep)>0){?><div class="row" style="margin:0px">
<?php foreach($dep as $dep_main){?>
<div class="col-md-2 dep-main"><img src="<?= base_url()?>uploads/icon/department_icon.png">
<span ><a href="<?= base_url()?>cat/subcategory?ID=<?= base64_encode($dep_main->id);?>" class="dep-title"><?= $dep_main->name;?></a></span></div>
<?php }?>
</div>
<?php }?>
</div>
			...
			<div class="toolbar" style="border-bottom:1px solid #8181812e">
              <div class="view-mode">
                <ul>
                  ...
                  <li > <a href="<?=base_url()?>cat/subcategory_list?ID=<?= $this->input->get("ID");?>"> <i class="fa fa-th-list"></i> </a> </li>
                </ul>
              </div>
              <div class="sorter">
...
?>
<span class="arrange"><a href="<?=base_url()?>cat/subcategory?ID=<?= $this->input->get("ID");?>" style="color:#003b67db">الاحدث</a></span>
<?php } else {?><span class="arrange">الاحدث</span><?php }?>
<span>|</span>
<?php
if($this->input->get("arrange")=="low_price"||$this->input->get("arrange")==""){
?>
<span class="arrange"><a href="<?=base_url()?>cat/subcategory?ID=<?= $this->input->get("ID");?>&arrange=heigh_price" style="color:#003b67db">الأعلى سعر</a></span>
...
?>
<span class="arrange"><a href="<?=base_url()?>cat/subcategory?ID=<?= $this->input->get("ID");?>&arrange=low_price" style="color:#003b67db"> الأقل سعر</a></span>
<?php } else if($this->input->get("arrange")=="low_price"){?><span class="arrange"> الأقل سعر</span><?php }?>		
            ...
            <a href="<?=base_url()?>cat/grid?ID=<?= $this->input->get("ID");?>&featured=no" class="adv_link">كل الأعلانات</a></span>
            <?php }else{?>
              <span style="padding:5px;font-size:16px;"><a href="<?=base_url()?>cat/grid?ID=<?= $this->input->get("ID");?>&featured=yes" class="adv_link">اعلانات مميزة</a></span>
